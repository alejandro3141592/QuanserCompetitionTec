...
class SteeringController:
    ...
    def update(self, p, th, speed):
        wp_1 = self.wp[:, np.mod(self.current_waypoint_index, self.N-1)]
        wp_2 = self.wp[:, np.mod(self.current_waypoint_index+1, self.N-1)]
        # print(wp_1)
        v = wp_2 - wp_1
        ...
        if s >= v_mag:
            if  self.cyclic or self.current_waypoint_index < self.N-2:
...
def controlLoop():
    ...
    global KILL_THREAD, x_global, y_global
    u = 0
    delta = 0
    ...
    with qcar, gps:
        ...
        while (t < tf+startDelay) and (not KILL_THREAD):
            ...
            # cameraCSI.readAll()
            # image = cameraCSI.csi[3].imageData
            qcar.read()
            # cv2.imshow("Front Image", image )
            if enableSteeringControl:
                
                if gps.readGPS():
                    x_global = gps.position[0]
                    y_global = gps.position[1]
                    y_gps = np.array([
                        gps.position[0],
                        gps.position[1],
                        gps.orientation[2]
...
def viewImage():
    global v_ref, x_global, y_global 
    csiReader = CSIReader()
    timer_started = False
    alto_seen = False
    tf = 3
    ...
    while(not KILL_THREAD):
        csiReader.readCamera()
        csiReader.viewImage()
        o_image = csiReader.image
        image =cv2.resize(o_image, (224, 224))
        ...
        if (index==2):
            ...
            print(x_global, end=', ')
            print(y_global)
            if (x_global > 2.02 and x_global < 2.32) and (y_global > 2.5 and y_global < 2.7):
                near_first = True
            else:
                near_first = False

            if (x_global > -0.6 and x_global < -0.4) and (y_global > 4.3 and y_global < 4.7):
                near_second = True
            else:
                near_second = False
...
if __name__ == '__main__':
    ...
    imageViewrThread = Thread(target=viewImage)
    imageViewrThread.start()
